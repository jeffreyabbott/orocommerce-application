...
use EDI\Bundle\DonationApplicationBundle\Form\Type\DonationApplicationWifeSocialandEduHistoryType;
...
class DonationApplicationEmbWifeSocialEduHistController extends AbstractController
{

	/**
	 * @Route("/", name="edi_donation_socialeduhist_wife_frontend", options={"frontend"=true})
	 * @Layout
	...
	public function createAction(Request $request, DonationApplicationPagingButtonsHelper $buttonsHelper)
	{
		...
		$emb_social_edu_hist = new EmbWifeSocialAndEduHistCombo();
		$emb_social_hist = new EmbWifeSocialHistory();
		$emb_edu_hist = new EmbWifeEduHistory();
	...
	/**
	 * Route("/update", name="edi_donation_socialeduhist_wife_frontend", options={"frontend"=true})
	 * Layout
	 * return array|RedirectResponse
	 */
	public function updateAction(EmbWifeSocialAndEduHistCombo $emb_wife_social_and_edu_hist_combo, Request $request, DonationApplicationPagingButtonsHelper $buttonsHelper)
	{
		return $this->update($emb_wife_social_and_edu_hist_combo, $request, $buttonsHelper);
	}

	private function update(EmbWifeSocialAndEduHistCombo $emb_wife_social_and_edu_hist_combo, Request $request, DonationApplicationPagingButtonsHelper $buttonsHelper)
	{
		$form = $this->createForm(
		DonationApplicationWifeSocialandEduHistoryType::class,
			$emb_wife_social_and_edu_hist_combo
		);
...
*/
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			$entityManager->persist($emb_wife_social_and_edu_hist_combo);
			$entityManager->flush();

			return $this->get('oro_ui.router')->redirectAfterSave(
				array(
					'route' => $nextAction, //Change this route
					'parameters' => array('id' => $emb_wife_social_and_edu_hist_combo->getApplicationId()),
				),
...
*/
		$returnThis =  ['data' => ['donation_wife_socialeduhist_form' => $form->createView(), 'entity' => $emb_wife_social_and_edu_hist_combo]];
