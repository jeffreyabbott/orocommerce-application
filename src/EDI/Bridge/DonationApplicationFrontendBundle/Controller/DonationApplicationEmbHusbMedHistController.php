...
use EDI\Bundle\DonationApplicationBundle\Form\Type\DonationApplicationHusbMedHistType;
...
class DonationApplicationEmbHusbMedHistController extends AbstractController
{
	...
	/**
	 * @Route("/", name="edi_donation_medhist_husb_frontend", options={"frontend"=true})
	 * @Layout
	...
	public function createAction(Request $request, DonationApplicationPagingButtonsHelper $buttonsHelper)
	{
		...
		$emb_medhist = new EmbHusbMedHist();

		return $this->update($emb_medhist, $request, $buttonsHelper);
	}

	/**
	 * Route("/update", name="edi_donation_medhist_husb_frontend", options={"frontend"=true})
	 * Layout
	 * return array|RedirectResponse
	 */
	public function updateAction(EmbHusbMedHist $emb_husb_med_hist, Request $request, DonationApplicationPagingButtonsHelper $buttonsHelper)
	{
		return $this->update($emb_husb_med_hist, $request, $buttonsHelper);
	}

	private function update(EmbHusbMedHist $emb_medhist, Request $request, DonationApplicationPagingButtonsHelper $buttonsHelper)
	{
		$form = $this->createForm(
			DonationApplicationHusbMedHistType::class,
			$emb_medhist
		);
...
*/
		$returnThis =  ['data' => ['donation_husb_medhist_form' => $form->createView(), 'entity' => $emb_medhist]];
